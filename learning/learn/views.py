...
def about(request):
    w = Registration.objects.get(User_role='admin')
    k = Registration.objects.filter(User_role='teacher')
    return render(request, 'about.html', {'w': w,'k': k })
...
def add_blog(request):
    if request.method == 'POST':
        ...
        fs = FileSystemStorage()
        fs.save(photo.name, photo)
        Date_blog = request.POST.get('Date_blog')
        b = Blogs()
        b.Name = Name
        b.Blog_content = Blog_content
        b.Image = photo
        b.Date_blog = Date_blog
...
def cont_tr(request):
    return render(request,'cont_tr.html')

def add_chapter_content(request):
    return render(request,'add_chapter_content.html')

def add_chapter_c2(request):
    return render(request,'add_chapter_c2.html')
...
def st_sub_selnew(request):
    return render(request,'st_sub_selnew.html')

def st_sub_selnew1(request):
    return render(request,'st_sub_selnew1.html')
...
def acc_chapter2(request):
    return render(request,'acc_chapter2.html')

def st_progress(request):
    st1 = Registration.objects.get(id=request.session['logg'])
    hgh1 = Enrollment.objects.filter(Teacher_email=st1.Email)

    hgh=[]
    for i in hgh1:
        if i.Student_email not in hgh:
            hgh.append(i.Student_email)
    dd = Learning_progress.objects.filter(Teacher_email=st1.Email)
    return render(request, 'st_progress.html', {'hgh': hgh,'dd': dd })
...
def exam_result(request):
    ...
    hh = Registration.objects.get(id = request.session['logg'])
    return render(request,'exam_result.html',{'gt':gt,'hh':hh})
...
def del_cert(request):
    gf=Enrollment.objects.all().exclude(Certificate='')
    return render(request,'del_cert.html',{'gf':gf})


def delete_cert(request, id):
    gf = Enrollment.objects.get(id = id)
    gf.Certificate = ''
    gf.save()
    messages.success(request, 'Certificate Delete Successfully')
    return redirect('del_cert')
...
def chap_content(request):
    gt = Registration.objects.filter(User_role='teacher')
    dm = Subject.objects.all()
    return render(request, 'chap_content.html', {'gt': gt, 'dm': dm})
